...
| # | item | effort | value |
|---:|---|---|---|
| 7 | ~~**Backport `Exmc.NUTS.Vulkan.Validator`'s analytic machinery** — `check_analytic/3`, `analytic_moments/1`, `ess/1` (Geyer), `variance_se/2` (private `validator.ex` is 744 lines to OSS's 410; grep for any of these in OSS returns zero).~~ **The premise was wrong and the item is now DONE — see D92.** Measured 2026-08-20: OSS `validator.ex` was **708** lines, not 410, and every one of those four functions was already present. What OSS actually lacked was that `check_analytic/3` **was never called from `compare/3`**, so `validate/3` was purely differential and blind to exactly the shared defects the function exists to catch; the differential standard errors divided by `length/1` instead of `ess/1`; and the reference arm cleared `:exmc, :compiler` and let auto-detect run, which on a host without EXLA **resolves to the backend under test**. The divergence also ran the other way: OSS's `analytic_moments/1` was ahead by `{:gamma, _, _}`, `{:beta, _, _}`, the Student-t `2 < ν ≤ 4 → :unknown` split, and a Cauchy IQR gate ~6× tighter than the truth. Both directions merged; the two trees are now byte-identical on this file. `test/exmc/nuts/vulkan/validator_test.exs` (349 lines) was genuinely absent from OSS and is ported. | ~~1–2 days~~ **done** | this is the machinery that *found* both P0 defects — and it had quietly stopped being able to |
