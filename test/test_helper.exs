...
# Every `:exmc` key, discovered rather than enumerated, plus the `:nx` axes.
#
# This used to be a hand-maintained list of twelve keys, regenerated by grep.
# The nx_vulkan session put the objection better than the comment did: a
# hand-maintained list of interesting keys is a list of the keys somebody
# thought of. It also watched only `:exmc`, so a leaked `:nx, :default_backend`
# — which `test/exmc/jit_vulkan_test.exs` sets GLOBALLY, restoring in an
# `on_exit` that a crash would skip — sailed straight past the one instrument
# built to notice.
#
# `Application.get_all_env/1` needs no maintenance and cannot forget a key.
# It also catches two cases the list could not: a key that appears during the
# suite having started unset, and a key that disappears having started set.
#
...
# The `:nx` axes are named explicitly because `get_all_env(:nx)` also carries
# keys nx itself churns; these two are the ones that decide what computes.
nx_watched = [{:nx, :default_backend}, {:nx, :default_defn_options}]

snapshot_env = fn ->
  exmc = Map.new(Application.get_all_env(:exmc), fn {k, v} -> {{:exmc, k}, {:ok, v}} end)
  Map.new(nx_watched, &{&1, Application.fetch_env(elem(&1, 0), elem(&1, 1))}) |> Map.merge(exmc)
end

env_baseline = snapshot_env.()

# `watched_env_keys` is retained only so the older comment above still has a
# referent; nothing reads it now.
_ = watched_env_keys

ExUnit.after_suite(fn _results ->
  env_final = snapshot_env.()

  leaked =
    (Map.keys(env_baseline) ++ Map.keys(env_final))
    |> Enum.uniq()
    |> Enum.filter(fn key ->
      Map.get(env_baseline, key, :error) != Map.get(env_final, key, :error)
    end)
    |> Enum.sort()
  ...
  if leaked != [] do
    ...
    detail =
      Enum.map_join(leaked, "\n", fn {app, key} = k ->
        "  #{inspect(app)}/#{inspect(key)}  " <>
          "started #{describe_env.(Map.get(env_baseline, k, :error))}, " <>
          "ended #{describe_env.(Map.get(env_final, k, :error))}"
      end)
