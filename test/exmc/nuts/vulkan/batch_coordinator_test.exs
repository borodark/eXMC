defmodule Exmc.NUTS.Vulkan.BatchCoordinatorTest do
  ...
  describe "live coord — partition + flush dispatching" do
    ...
    test "single chain request to a dead-Dispatch path surfaces fallback", %{coord: coord} do
      # The :synthesised meta carries an SPV path that doesn't exist
      # ("/tmp/fake.spv"). Dispatch.chain_batch will raise; the
      # coordinator's try/rescue must surface :fallback rather than
      # crash the coord.
      ...
      result =
        BatchCoordinator.request_synth_chain(coord, @synth_meta, q, p, im, obs, 0.05, 4, 1)
