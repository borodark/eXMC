...
## 86. nx_vulkan backend runs f64 by default (not f32)
- Decision: When the active compiler is `Nx.Vulkan`, the sampler runs the leapfrog + gradient path at **f64**, not f32. `Exmc.JIT.precision/0` returns `:f64` for Vulkan (matching EXLA/Evaluator), the default backend is `Nx.Vulkan.VulkanoBackend`, and defn dispatch goes through `Nx.Defn.Evaluator` with that backend set globally (VulkanoBackend implements the compute callbacks; there is no dedicated Vulkan `Nx.Defn.Compiler` — no kernel fusion in v0.1). An operator can force f32 for throughput or on hardware without f64 support via `config :exmc, :force_precision, :f32`.
- Rationale: The f32 chain-shader path silently collapsed on ill-conditioned models — for a multi-scale model, f32 precision loss accumulating over the leapfrog trajectory drove step size and posterior sd to zero (100% divergence) regardless of mass-matrix init. f64 absorbs the leapfrog dynamics and produces a bit-exact match with the CPU/EXLA path on every GPU tested. Defaulting to correctness and making f32 the opt-in escape hatch is the safer contract.
- Assumption: VulkanoBackend supports f64 on the target GPUs. `Nx.Vulkan.has_f64?/0` is intentionally NOT consulted — it probes a legacy context that returns false on hardware where vulkano f64 works end-to-end; gating on it would keep the f32 default on working GPUs. A genuinely f64-lacking device is handled by the operator setting `force_precision: :f32`.
- Evidence: Full suite green on this port — 11 doctests, 8 properties, 357 tests, 0 failures, 3 excluded (EXLA f64 path). posteriordb: 33/33 gold-standard models pass (100%), max mean-error ≤ 0.18, sd-ratio ≤ 1.48.
- Implication: `Exmc.JIT` is the single backend-selection seam: auto-detect order is EXLA > EMLX > Vulkan > Evaluator, each overridable via `config :exmc, :compiler`. Vulkan is the cross-platform GPU path (FreeBSD NVIDIA, Linux non-CUDA, macOS via MoltenVK) when EXLA/EMLX are absent; EXLA still wins on hosts that have it.

## 87. Vulkan f64 coverage via the vulkano synth chain-shader path
- Decision: f64 GPU chain-shader coverage is achieved by routing synthesisable models through the custom-synth path (`Exmc.NUTS.ChainShaderCodegen.detect_meta/1` → `Exmc.NUTS.CustomSynth`), which emits a fused f64 leapfrog chain shader (GLSL) dispatched via `Exmc.NUTS.Vulkan.Dispatch` / `BatchCoordinator`. `Compiler.compile_for_sampling/2` now returns a 6-tuple whose last slot is the detected `chain_meta` (`nil` when unsupported). A **Plan B′ guard** raises `Exmc.SynthUnsupportedError` at compile time if the Vulkan compiler is selected but the IR is not synthesisable, rather than silently falling through to slow per-op host fallbacks (escape hatch: `config :exmc, :allow_vulkan_perop_sampling, true`). Multi-chain GPU concurrency is bounded by `Exmc.NUTS.Vulkan.Scheduler`, a VRAM-aware permit semaphore that degrades to direct execution when not started (no supervision tree required).
- Rationale: Extending hand-written per-family fused SPVs to f64 does not scale across the distribution zoo. Synthesising the chain shader from the IR covers standard-family priors (Normal/StudentT/Cauchy/Exponential/HalfNormal/Weibull, plus multi-RV `:synthesised` layouts) with an optional Custom likelihood in one code path. Failing loud at compile time (Plan B′) surfaces unsupported models immediately instead of as a 100×-slower silent regression.
- Assumption: The cold-start mass matrix benefits from a per-RV closed-form heuristic (`prior_inv_mass_per_rv/2`) seeded from prior variances; identity init was catastrophic for models whose parameters span orders of magnitude. Phase-II Welford refinement still runs and corrects the estimate — the heuristic only supplies a sensible starting neighborhood.
- Evidence: Ported and merged surgically into the OSS tree with zero proprietary (trading/license) or MLX bleed; compiles clean; full suite + posteriordb green (see D86). Backend-selection and per-op fallback are guarded, so EXLA/EMLX hosts see no behavior change.
- Implication: The public `compile_for_sampling/2` contract is now a 6-tuple; internal consumers (`Sampler`, `Distributed`, tests) accept 4/5/6-tuple shapes for backward compatibility. The `:exmc_chain_meta` / `:exmc_warmup_phase` process-dictionary flags are the routing seam between the sampler and `Tree.dispatch_multi_step`.

## 88. nx_vulkan W5 is scoped by census, and the integer bucket is three gaps not one
- Decision: W5 ("integer elementwise / compare / select / reduce kernels", nx_vulkan `MISSION.md` §7) is scoped and priced before any kernel is written, by running all 843 `doctest Nx` under `NXV_HOST_FALLBACK=raise` and reading the op and dtype out of every `Nx.Vulkan.HostFallbackError`. The 357 doctests the residency register files under `@integer_dtype` partition into three classes that need different work: **195 dtype-gated** (a shader exists for f32/f64 and the selector returns `nil` for integers — six functions ending in `defp …(_), do: nil`: `binary_spv/1`, `unary_spv/1`, `compare_spv/1`, `select_spv/1`, `reduce_spv/2`, `window_reduce_spv/1`); **138 with no GPU path at any dtype** (in `@host_fallback_unary_ops` / `@host_fallback_binary_ops` or transferring unconditionally — 67 of these ride the same kernels and need only an op code, the other 71 are separate items); and **24 shape- or residency-gated**, integer only because Nx's own doctests are `{:s, 32}`. Scope is 32-bit: `int`/`uint` in core GLSL 450, no `Int64` capability and no 8/16-bit storage extension. Full write-up in `nx_vulkan/NEXT.md` §1.2 at `932c2b9`.
- Rationale: The register scores the bucket at 357 and MISSION says W5 "empties it wholesale". Those are not the same claim, and the difference is 162 doctests of work that writing integer shaders does not do. The same census method was what made W4 pay — it turned twelve opaque `Nx.Block.*` fallbacks into three named gaps and identified an axis > 0 `concatenate` shader as the single highest-leverage missing kernel, which then moved five ops at once. Scoping by measurement rather than by the bucket label is the cheaper half of the work.
- Assumption: The allowlist can be used as a simulator — excuse a family of callbacks, re-run the strict pass, and the residency that comes back is what that family's shaders would buy. This errs in both directions and roughly cancels: it over-counts by ~14 because excusing `window_sum/4` excuses its f32 instances too (those belong to `@float_residency_gap`), and it under-counts because an allowlisted op still computes on the host and returns a host tensor, so residency-gated ops downstream of one still report. The second effect is visible: `concatenate/3` rises from 7 to 34 in the residual purely as an artifact of `concat_nd`'s `all_vulkano?` gate refusing operands that real shaders would have kept resident.
- Evidence: Measured on super-io (RTX 3060 Ti, driver 580.178.04 matched) 2026-08-17. Simulated residency: today **398/843 (47.2%)**; integer elementwise + compare + select **535/843 (63.5%)**; plus integer axis- and window-reduce **624/843 (74.0%)**; plus integer `dot` **641/843 (76.0%)**. **+243 doctests.** Of the 195 dtype-gated, 168 are `{:s, 32}`, 23 are `{:u, 8}` compare *outputs* from s32 inputs, 4 are `{:s, 8}`. Across all 445 first-fallbacks there are five `{:s, 64}` rows, four `{:u, 32}`, two `{:s, 16}`. Under `:warn` all 843 run to completion and report **759** fallbacks rather than 445, and that ordering differs sharply: integer `max/3` is 13th by first-fallback count and **89** by demand — the most-called missing kernel in the API — with `add/3` at 70 and `concatenate/3` at 57.
- Implication: W5 is `.comp` files plus one clause per selector; **the `native/` Rust tree does not move**, because every dispatch entry point already takes the SPIR-V path as a parameter (`apply_binary/6`, `apply_unary/5`, `apply_compare/7`, `apply_select/8`, `reduce_axis/7`, `window_reduce/7`, `matmul32/7`). Seven new shaders take the repo's shader invariant from 57 ↔ 57 to 64 ↔ 64. Six integer semantics traps were measured against `Nx.BinaryBackend` rather than recalled, and each would produce a bit-exact disagreement: s32 `sum` **wraps** (`2e9 + 2e9 → -294967296`), so an s32 reduce must NOT widen to a double accumulator the way the f32 shader does; `{:s, 8}` `sum` widens to `{:s, 32}` (which is why `reduce_spv/2` is keyed on the in/out pair); `{:s, 8}` `multiply` wraps at 8 bits (`100 * 100 → 16`); `remainder` takes the sign of the dividend where GLSL's `%` is undefined for negative operands; `quotient` truncates toward zero with the same caveat; and `count_leading_zeros(0)` is 32 where `findMSB` returns -1. `Nx.divide` on two integers returns `{:f, 32}`, so no integer divide op code is needed at all. Sequencing: integer elementwise first and stop to measure — it is 137 doctests on its own, every later tier reuses its shaders, and all six traps live in it. The largest non-decided residual after W5 is `indexed_put/5` at 22 doctests: an unconditional host fallback at every dtype, the natural sibling of the `gather.comp` that has existed all along, and where `Nx.LinAlg.invert/1` dies. It deserves a W-number of its own.

## 89. A green differential test is not evidence: the observed-model fix is confirmed against the closed form, across variants
- Decision: The `6c1589a` fix (`fix(synth): every observed node summed the WHOLE obs buffer`) is not accepted on the strength of `integration_test.exs:646` going green. It is confirmed by `bench/observed_model_evidence.exs` — **nine model shapes × four seeds × both arms**, each row scored against the **closed-form conjugate posterior** rather than against the other arm. The variant set is deliberately not permutation-invariant (`"scalar 3 obs, sigmas 1/2/3"`, `"scalar 5 obs, sigmas 1..5"`, `"scalar 4 obs, sigmas .5/1/2/4"`), because a whole-buffer sum is invisible to any model whose observations are exchangeable in the defective term. Every run carries a **vacuity guard** — `:erlang.trace_pattern({Exmc.NUTS.Vulkan.Dispatch, :chain, 8}, true, [:call_count])` after a `Code.ensure_loaded!/1`, so a `gpu` column of 0 on the `:vulkan` arm fails the row instead of passing it silently.
- Rationale: The whole class of defect here is one both arms could share, and a differential harness is structurally blind to exactly that. The frozen chain (1 distinct draw in 500, sd 3.29e-14) had also been "passing" a tolerance for a full session, which is the second reason one green assertion on one seed is not a result. Scoring against the analytic posterior removes the shared-mode blindness; sweeping variants removes the single-seed luck; the trace counter removes the possibility that the GPU arm never ran.
- Assumption: `Code.ensure_loaded!/1` before `trace_pattern/3` is load-bearing — the pattern silently matches nothing against a module that has not been force-loaded, and a vacuity guard that is itself vacuous is worse than none. Also assumed: two bit-identical rows across seeds are expected for this model family (same data, same adapted trajectory) and are **not** a shader-cache collision; the sweep's own note says so, so the next reader does not chase it.
- Evidence: **72 of 72 rows within tolerance.** Worst mean error 0.091, worst sd error 8.1%, fewest distinct draws 447/500. The frozen-chain signature appears in no row. The scalar arm on the original failing model measures mean 3.9864, sd 0.5536, 479/500 distinct against analytic mean 3.99, sd 0.577. `EXMC_COMPILER=vulkan mix test`: 4 failures → 2 across this and D90. Table in [`docs/OPEN_VULKAN_OBSERVED_MODEL.md`](docs/OPEN_VULKAN_OBSERVED_MODEL.md), raw output and host in [`bench_results/OBSERVED_MODEL_EVIDENCE.md`](bench_results/OBSERVED_MODEL_EVIDENCE.md).
- Implication: The old "Scope note" in `OPEN_VULKAN_OBSERVED_MODEL.md` is marked superseded on both counts — `:vulkan` is not the default (`Exmc.JIT.auto_detect/0` prefers EXLA where it exists) and the model is no longer open. For future synth work the shape of the harness is the reusable part: analytic scoring + non-exchangeable variants + a `call_count` guard on the dispatch entry point. Operationally: `epmd -daemon` before any sweep, or `distributed_test.exs` contributes two failures that have nothing to do with the code.

## 90. Crash recovery is hoisted above dispatch selection, and a recovered step is neither a divergence nor an adaptation signal
- Decision: Three changes to the fault-tolerant path. (1) `safe_build_subtree/12` is replaced by `with_supervision/7` in `lib/exmc/nuts/tree.ex`, **hoisted to wrap whichever dispatch actually runs** rather than sitting on one of the three tree implementations. (2) `divergent_placeholder/4` returns `n_steps: 0`, not `trunc(:math.pow(2, depth))` — those leaves were never integrated. (3) `nuts_step_warmup` returns `{new_state, accept_stat, recovered}`; a recovered step is excluded from the divergence count (`if result.divergent and not recovered`) and is skipped by dual averaging via `maybe_adapt(da_state, _accept_stat, true), do: da_state`.
- Rationale: The reported defect was one bug. It was three, and two of them were vacuities that hid the first. Supervision was **accepted, reported, and did nothing** wherever the speculative buffer was live — the wrapper sat on a branch that path never took. The `recoveries` counter was **structurally zero**: only the sampling-phase function read the flag, and every injected crash was a warmup crash. And a placeholder fed to dual averaging is a fabricated rejection at a fabricated `n_steps`, which drives ε to zero and reports the resulting frozen chain as a successful run with a high divergence count — failure presented as diagnostics.
- Assumption: A crashed subtree carries no information about whether the step size was too large, so withholding it from dual averaging is the correct accounting, not a convenience. This assumption is only safe while crashes are rare; a run where most steps are recovered is one where ε stops adapting at all, and the `recoveries` counter is what makes that visible. `bench/crash_recovery.exs` prints `VACUOUS` if injection was on and nothing was recovered, and `MISREPORTED` if placeholders ≠ recoveries — both failure modes this entry exists because of.
- Evidence: `:vulkan`, injection at depth 3, before → after: variance **1.45e-15 → 0.8807**, ε **2.41e-11 → 0.8102**, recoveries **0 → 29**, divergences **178 → 3**. Both no-injection controls are **bit-identical** before and after, so nothing on the clean path moved. The vacuities, measured: on `:none` the injector was consulted **1463 times and the placeholder hit zero times** (a well-adapted sampler never builds trees that deep — the test had been green on two backends for months with recovery never once running), and **861 subtree builds entered the supervision wrapper zero times**. Elsewhere, **176 placeholders built while `recoveries: 0` was reported**. `EXMC_COMPILER=vulkan mix test`: 4 failures → 2. Matrix in [`bench_results/CRASH_RECOVERY.md`](bench_results/CRASH_RECOVERY.md).
- Implication: `test/fault_tolerant_test.exs` injects at **depth 1**, not depth 3, so the path fires on every backend, and asserts `Map.get(stats, :recoveries, 0) > 0` — a test that cannot distinguish "recovered" from "never ran" is not a test of recovery. A second test, `"crash recovery does not destroy the adapted step size"`, pins `eps > 0.01` and `stats.divergences < recoveries`. The general rule this generalises to, and the reason it is filed here: **a vacuous check reads exactly like a passing one**, so any test of a rare path needs a counter proving the path was taken.

## 91. Posterior assertions must be power-checked, not just toleranced — `assert_posterior!/3` and draw counts sized to ESS
- Decision: Statistical assertions on posteriors go through `Exmc.TestHelper.assert_posterior!/3` (`test/test_helper.exs`), which does two things in a fixed order: it checks the analytic moments **first**, then applies a power gate that flunks `INCONCLUSIVE` if the chain cannot resolve the error size claimed. `:unknown` from `analytic_moments/1` flunks rather than passing. Draw counts are sized from ESS and skew rather than by habit: conjugate N-N 500→**4000**, Gamma 200→**8500**, Exponential 300→**12000**, Beta 400→**5000**, Lognormal 300→**10000**, Normal(0,1) 500→**4000**.
- Rationale: The audit asked "which tolerances would accept a 20% variance error" and the answer was **all of them** — not one statistical assertion in the suite could see one, including the assertion 0.3.1 added as the fix for exactly that defect, which resolved **37.8%** against a defect that was **37.8%**. The cause was sample size, not tolerance width, so a pure tolerance rewrite would have changed nothing. `check_analytic/3` sizes its band from the chain's own ESS, so it can never be *too tight* — and therefore degrades silently to nothing on a short chain, saying so nowhere. The power gate is the missing half: a band that adapts to the data must also refuse to report a pass it cannot support.
- Assumption: The check order is load-bearing and was **wrong on the first attempt**, caught by probing the helper against a deliberately inflated chain: the resolution figure is computed from the sample's own 4th moment, so a 20%-inflated chain inflated its own `detectable` from 20.6% to 23.0% and got reported "INCONCLUSIVE" rather than "variance wrong". Moments first; the power gate only ever governs whether a *pass* is allowed to stand. `test/tolerance_gate_test.exs` pins this with three tests, including the 0.3.0 defect (`:math.sqrt(1.378)` inflation) asserting `analytic_variance failed` **and** `refute err.message =~ "INCONCLUSIVE"`.
- Evidence: NUTS yields ~**0.35 effective draws per draw** here (Geyer and `Diagnostics.ess` agree to 1%), so 500 draws buys ESS 178 and a 37.8% resolution limit. The correct standard error of a sample variance is `sqrt((mu4 - sigma^4)/n_eff)`, **not** `sigma^4 * 2/n_eff` — that assumes normality, and the ratio `(mu4 - sigma^4)/sigma^4` is 2 for a Normal but **8** for an Exponential, which is why the Exponential test needs 12000 draws where the Normal needs 4000. Two gates were wrong in the *other* direction: Student-t at df ≤ 4 has an infinite 4th moment (`when nu > 2 and nu <= 4, do: :unknown`) so the band was noise with a number on it, and the Cauchy IQR gate was ~6× too tight — `0.25 * tiqr / sqrt(n)` instead of `0.5 / (f * sqrt(n))` with `f_quartile = 1/(2π·scale)` — i.e. it would have **failed a correct sampler**. Full audit in [`docs/TOLERANCE_AUDIT.md`](docs/TOLERANCE_AUDIT.md).
- Implication: Draw counts must be sized against the **slower** arm, not the faster one: sizing every count to land at 17–19% on EXLA made the Lognormal test fail under `EXMC_COMPILER=vulkan` at **21.3%** — same model, same seed, same count, different ESS. Counts now target ~15% on the slow arm. `analytic_moments/1` gained `{:gamma, alpha, beta}` and `{:beta, a, b}`. StudentT(df=4) deliberately does **not** use the helper — an ESS-sized mean gate plus an analytic IQR gate with `f_quartile = 0.2719`, with a comment saying why no variance gate is possible there. Cost: the Vulkan sweep went 1047s → 1301s, which is what re-surfaced `poker_test.exs:228` — that test is **load-flaky, not fixed**, and there is no timeout containment on the Vulkan path at all.

## 92. The validator drifted in both directions, and the reconciliation document counted names instead of reading bodies
- Decision: `lib/exmc/nuts/vulkan/validator.ex` is reconciled **both ways** between the OSS and applications trees, and the two trees are now byte-identical on it and on `test/exmc/nuts/vulkan/validator_test.exs`. Private → OSS: `reference/0` + `run_reference/2` (which pin the reference compiler explicitly and raise if it ever resolves onto the candidate), `check_analytic/3` wired into `compare/3` on **both** arms, and `ess/1` rather than `length/1` as the denominator of every differential standard error. OSS → private: `analytic_moments/1` for `{:gamma, _, _}` and `{:beta, _, _}`, the Student-t `2 < ν ≤ 4 → :unknown` split, and `f_quartile` + `cauchy_quantiles/2` with the corrected `SE(IQR) = 0.5/(f·sqrt(n))`. `GATE1_RECONCILIATION.md` Batch D and `MISSION.md` §7 P1 item 7 are corrected in place rather than deleted.
- Rationale: Batch D said OSS's copy was 410 lines against private's 744 and that grepping OSS for `check_analytic/3`, `analytic_moments/1`, `ess/1` or `variance_se/2` "returns zero". It was 708 lines and all four were present. A later re-measurement caught the line count but repeated the method — it compared function *heads*, found "two names each way", and concluded "a rename plus one helper, not a missing subsystem". Both readings are what you get from counting names. `diff -u` on the two files is **624 lines**, and the difference is in the bodies.
- Assumption: Three of the private → OSS items are live defects rather than style, and this is the load-bearing assumption of the whole entry. `check_analytic/3` was **defined in OSS and never called from `compare/3`** — so `validate/3` ran mean, variance and KS, all differential, and was structurally blind to precisely the shared defects that function exists to catch, while its own moduledoc said otherwise. The differential standard errors divided by `length/1`, so a nominal 3σ gate operated near 1.7σ on chains with lag-1 autocorrelation of 0.33–0.38. And `run_exla/2` cleared `:exmc, :compiler` and let `Exmc.JIT.detect_compiler/0` fall through, which resolves to `Nx.Vulkan` on any host without EXLA — **the backend under test**. Private had already found and fixed that one (it cost three weeks of the FreeBSD fleet reporting that Vulkan agreed with Vulkan); OSS still had it.
- Evidence: Measured 2026-08-20 before any edit — OSS 708 lines / 26 `def`+`defp` heads / 24 names in common, against private's 744 / 26 / 24. Nothing in OSS calls `Validator.validate/3` outside a moduledoc example, which is why the vacuous reference arm never announced itself. The OSS→private items all date from `b2aefca` (2026-08-18) and therefore post-date the reconciliation document, so that half of the drift was hours old, not months. After the merge: `test/tolerance_gate_test.exs` + `test/nuts/p0_correctness_test.exs` 13 tests / 0 failures; `test/mclmc` + `test/integration_test.exs` 68 tests / 1 failure (`integration_test.exs:815`, the known wall-clock one); the ported `validator_test.exs` 28 tests / 0 failures / 9 excluded on a non-Vulkan run.
- Implication: OSS had **no `test/exmc/nuts/vulkan/` directory at all** — that part of Batch D was accurate, and it is the mechanism for the whole entry: an untested copy is free to drift, and drift in a *verification* module is invisible by construction, because the thing that would notice is the thing that broke. `validator_test.exs` is ported and gains three clauses pinning the Student-t ν ≤ 4 boundary. The four remaining private-only files in that directory (`batch_coordinator`, `bulkhead`, `chaos`, `server`) are named as an open gap rather than silently swept in. Separately: `GATE1_RECONCILIATION.md`, `PLAN_CORE_AND_APPLICATIONS.md` and `PLAN_SAMPLER_ROADMAP.md` govern both repos and are **untracked in either**, which is how a plan document gets to be wrong for four days without anyone noticing — that arrangement still needs an operator decision.
